...
// File Structure State: fileStructure is initialized with initialFileStructure, which likely represents the default structure of the file system (folders, files, etc.).
// Open Files State: initialOpenFiles retrieves the children of the fileStructure to set up the initial state of open files. If there are no children, it initializes as an empty array.
// Active File State: activeFile is initialized to the first open file (if it exists), allowing immediate access to a file for editing or viewing.
    ...
    // Purpose: This function toggles the isOpen state of a directory based on its id.
    ...
    // Purpose: This function collapses all directories in the file structure.
...
//             parentDirId: The ID of the parent directory where the new directory will be created.
// newDir: Either a string representing the name of the new directory or a FileSystemItem object.
// sendToSocket: A boolea
        ) => {
            ...
            // Purpose: This function creates a new directory within the specified parent directory.
            ...
            // toast.success("New Directory Created")
        )
    ...
    const renameDirectory = useCallback(
        (
            ...
            newDirName: string,
            sendToSocket: boolean = true,
            // sendToSocket: A boolean that determines whether to notify other clients about the rename via a socket.
        ): boolean => {
            const renameInDirectory = (
                ...
                if (directory.type === "directory" && directory.children) {
                    ...
                    if (isNameTaken) {
                        toast.success("Take another name")
    ...
    const createFile = useCallback(
        (
            ...
            // If the file is a string, we need to generate a unique name
            let baseName = file.split('.').slice(0, -1).join('.'); // Name without extension
            const extension = file.split('.').pop(); // Get the extension

            let uniqueName = baseName;
            let num = 1;

            // Check if the file already exists, and if so, create a unique name
            while (isFileExist(parentDir, uniqueName + (extension ? `.${extension}` : ''))) {
                uniqueName = `${baseName}(${num})`; // Generate new name
                num++; // Increment the number
            }

                newFile = {
                    id: uuidv4(),
                    name: uniqueName + (extension ? `.${extension}` : ''),
                    type: "file",
                    content: "",
                }
            } else {
                ...
                if (directory.type === "directory" && directory.children) {
                    const isNameTaken = directory.children.some(
                        (item) =>
                            item.type === "file" &&
                            item.name === newName &&
                            item.id !== fileId // Ensure we're not checking the current file
                    );
    
                    // If the name is taken, show a toast and return the original directory structure
                    if (isNameTaken) {
                        toast.error(`File name "${newName}" already exists.`);
                        return directory; // Return the directory as is without renaming
                    }
                    return {
                        ...directory,
                        children: directory.children.map((item) => {
...
//     useCallback is a React hook that helps optimize performance by memoizing a function. Here's how it works and why it's useful:

// Purpose of useCallback:

// It returns a memoized version of the callback function (handleDirUpdated in this case) that only changes if one of its dependencies changes.

// The first parameter is the function you want to memoize (handleDirUpdated).
// The second parameter is an array of dependencies ([updateDirectory]). This array tells React when to recreate the memoized function:
// If any value in this array changes, the handleDirUpdated function will be recreated.
// If updateDirectory (the dependency) stays the same between renders, handleDirUpdated will keep the same reference.
    ...
    // The useEffect hook establishes socket connections and event listeners when the component mounts and cleans them up when it unmounts, ensuring that the application remains responsive to real-time updates.
